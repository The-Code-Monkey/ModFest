...
public class AlienScrapOreFeature extends Feature<AlienScrapOreConfig> {
    private static final Block STONE = BlocksRegistry.METEOR_STONE;

    public AlienScrapOreFeature(Codec<AlienScrapOreConfig> config) {
        super(config);
    }

    @Override
    public boolean generate(ServerWorldAccess world, StructureAccessor structures, ChunkGenerator chunkGenerator, Random random, BlockPos pos, AlienScrapOreConfig config) {
    ...
    protected boolean generateVeinPart(ServerWorldAccess world, Random random, AlienScrapOreConfig config, double startX, double endX, double startZ, double endZ, double startY, double endY, int x, int y, int z, int width, int height) {
        int count = 0;
                                ...
                                for (int int_19 = int_13; int_19 <= int_16; ++int_19) {
                                    ...
                                    if (double_20 * double_20 + double_21 * double_21 + double_22 * double_22 < 1.0D) {
                                        ...
                                        if (!bitset.get(int_20)) {
                                            ...
                                            if (world.getBlockState(pos).getBlock() == STONE) {
